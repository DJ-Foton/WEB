...
class db
{
	...
	*/
	 ...
	 */
	public function DBUpdate($table_name, $item, $id)
	{
	           $insert_ask = "";
	 			 	...
	 			 	if ($insert_ask != "") $insert_ask .= ", ";
	 			 		...
	 			 		if (key_exists("value", $row))
	 			 				...
	 			 				$insert_ask .= "`$column` = $value_pom";
	 			 	}
	
	 			 	// 1) pripravit dotaz s dotaznikama
	 			 				$query = "update `$table_name` set $insert_ask where `id_$table_name` = $id;";
	 			 				//echo $query;
    ...
    public function DBDeleteR($table_name, $id_pokoj, $id_datum)
	{
	   // 1) pripravit dotaz s dotaznikama
	 		$query = "DELETE FROM `$table_name` WHERE `datum_id_datum` = $id_datum AND `pokoj_id_pokoj` = $id_pokoj";
